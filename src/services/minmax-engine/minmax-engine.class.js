...
export class MinmaxEngineService {
  ...
  async params (data) {
    ...
    const [paramsRes, covRes, branchRes, overrideRes] = await Promise.all([
      ...
      this._execSql(
        `SELECT cb.BRANCH, b.NAME AS BRANCH_NAME, cb.MARIME, cb.INCLUS, cb.ESTE_HQ, cb.ESTE_PODEA, cb.UPDATEDAT FROM CCCMINMAXBRANCH cb ` +
        `LEFT JOIN BRANCH b ON b.BRANCH = cb.BRANCH AND b.COMPANY = ${COMPANY} ORDER BY cb.BRANCH`,
        [], token
      ),
