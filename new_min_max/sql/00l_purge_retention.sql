...
BEGIN
    ...
    DECLARE @PurgeResult TABLE (RUNID INT, DELETED_WEEK INT, DELETED_WINSOR INT, DELETED_DET INT);
    ...
    BEGIN
        ...
        BEGIN TRY
            INSERT INTO @PurgeResult (RUNID, DELETED_WEEK, DELETED_WINSOR, DELETED_DET)
            EXEC dbo.sp_MinMaxEngine_PurgeRun @Company = @Company, @RunId = @RunId, @BatchSize = @BatchSize;

            INSERT INTO @Result (RUNID, DELETED_WEEK, DELETED_WINSOR, DELETED_DET, ERRORMSG)
            SELECT RUNID, DELETED_WEEK, DELETED_WINSOR, DELETED_DET, NULL FROM @PurgeResult;
        END TRY
        BEGIN CATCH
            DECLARE @PurgeError NVARCHAR(500) = LEFT(ERROR_MESSAGE(), 500);

            INSERT INTO @Result (RUNID, DELETED_WEEK, DELETED_WINSOR, DELETED_DET, ERRORMSG)
            VALUES (@RunId, NULL, NULL, NULL, @PurgeError);
    ...
    SELECT RUNID, DELETED_WEEK, DELETED_WINSOR, DELETED_DET, ERRORMSG
    FROM @Result
    ORDER BY RUNID ASC;
END;
