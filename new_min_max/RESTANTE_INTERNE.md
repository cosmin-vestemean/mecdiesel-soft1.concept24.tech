...
| 6 | P16 | Linii de vânzare materializate o singură dată per `RUNID`; `Classify` și `ClassifyGroup` citesc același set înghețat | [PLAN_IMPLEMENTARE.md](PLAN_IMPLEMENTARE.md) §272 — decizie implementată și testată local | Tabelul `CCCMINMAXSALES` (14 coloane + RUNID, cheie clustering pe RUNID) materializează o singură dată liniile de vânzare eligibile; ambele faze de clasificare citesc snapshotul | Deployat. RUNID 30: `vz_grp_det = 0/602`, confirmând reconcilierea DET↔GRP pe același input înghețat. Închiderea formală așteaptă o probă controlată cu modificare ERP între Classify și ClassifyGroup; RUNID 27/28 rămân dovada driftului anterior (3/602 vs 0/602). |
| 7 | P17 | Aplicarea politicii de retenție: cineva trebuie să cheme `PurgeRun` conform `RETENTIE_DET_SESIUNI` | FAZA6 §6 — mecanism și politică declarate, ambele există | Ultima verigă: `purgeRun` e în serviciu și în `socketConfig.js`, dar **lipsește din store și nu are niciun declanșator** în UI sau automat | **Măsurat 17.09.2026:** 16 sesiuni `DONE` (RUNID 13–28) rețin `CCCMINMAXDET` 10.256 MB + `WEEK` 420 MB + `WINSOR` 116 MB ≈ **10,8 GB**, la o politică declarată de **2 sesiuni** (≈1,3 GB). Degradează tăcut — costul real e fereastra de backup a bazei ERP |

> Nota P16 (măsurată 17.09.2026, RUNID 27 vs 28): `Classify` apelează `ufn_MinMaxSalesLines(..., NULL)` și stabilește `AZI`; `ClassifyGroup` reapelează funcția cu `@AziInghetat`, dar pe sursa vie, după ~1 minut. Un document introdus între cele două citiri, cu `TRNDATE` în fereastră, intră în a doua fază și nu în prima. Proba: în fereastra lui RUNID 27 s-au modificat 1.313 linii ERP (24 de grupe), din care documente din `MTRGROUP=30` la 11:36:48 — exact delta `+1,0` măsurată pe filiala 2800; în fereastra lui RUNID 28 doar 35 de linii, iar `vz_grp_det` iese 0/602. **Alege materializarea liniilor, nu derivarea grupei din `CCCMINMAXDET`**: a doua variantă ar face `vz_grp_det` o tautologie și ar distruge exact invarianta care a prins abaterea 1c (313/602 pe RUNID 25). `Compute` nu e afectat — citește deja numai stare persistată.
...
- **Plafonul ADO de 60 s nu este o constrângere de arhitectură — subiect închis, nu îl redeschide.** Orice lucrare grea se rulează sub SQL Server Agent, tiparul deja folosit: `runPhases` pornește jobul `MEC_MinMaxEngine_RunPhases_<company>`, iar `Classify → ClassifyGroup → Compute → FinishRun` rulează acolo fără limită de timp (dovada: RUNID 15 are `Classify` la 148 s). Plafonul se aplică **exclusiv** apelurilor sincrone `X.RUNSQL` din AJS — `setup()`, `StartRun`, `explain`/preview — iar dacă o operație depășește 60 s, răspunsul corect este *mutarea ei sub Agent*, nu fragmentarea artificială a logicii. Detaliu de reținut înainte de a planifica: jobul existent are **formă fixă** (un singur pas, comandă hardcodată `EXEC dbo.sp_MinMaxEngine_RunPhases`), deci nu acceptă SQL arbitrar — o lucrare nouă se atașează fie extinzând `RunPhases`, fie printr-un al doilea job de formă fixă.
- Limita reală pentru punctele 2 și 4 este **volumul rulării** (tempdb, dimensiunea `CCCMINMAXDET`, fereastra de backup), nu un plafon de execuție.
