...
| 6 | P16 | Linii de vânzare materializate o singură dată per `RUNID`; `Classify` și `ClassifyGroup` citesc același set înghețat | [PLAN_IMPLEMENTARE.md](PLAN_IMPLEMENTARE.md) §272 — decizie declarată, neimplementată | Substratul comun; azi fiecare fază reconstruiește `#SalesLines` din sursa vie, la ~1 minut distanță | **Corectitudine, nu cosmetică.** Cauza dovedită a abaterilor `vz_grp_det` (RUNID 27: 3/602). Ancora `AZI` înghețată fixează marginile ferestrei, nu conținutul |
