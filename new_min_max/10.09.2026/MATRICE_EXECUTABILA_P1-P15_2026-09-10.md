...
| P2 | Netting pe `TRDR`, nu `TRDR.CODE`; HQ din neturi de filială (D13) | M/U | Secțiunea 7 din [01_classify.sql](../sql/01_classify.sql): filialele și HQ sunt materializate în `#ClientWindowTotals`, apoi agregate în `#BranchWindowTotals` | **DECIS și implementat local (16.09.2026):** cheia rămâne `TRDR`; HQ este scope propriu al cererii. Pentru HQ, neturile brute din toate filialele se însumează per `TRDR × MTRL × fereastră`, apoi se aplică clip-ul la zero. Nu se însumează VZ-urile filialelor deja clip-uite. Seria săptămânală, `VAL_52S`, sigma și CV rămân neschimbate direct; lifecycle/ABC/XYZ/clasă și MIN/MAX pot deriva în HQ prin noile VZ. Pipeline-ul de grupă rămâne intenționat în afara P1/P2 | — | T1 + regresie cross-branch `+10/-6`: filiale `10/0`, HQ `4`; gardă weekly pe `SUM(bw.QTY)` | După P1; implementat local, necesită deploy și validare SQL live |
...
- **P2, cheia și scope-ul HQ (10.09/16.09.2026)**: netting pe `TRDR`, nu pe `TRDR.CODE`; `NETTING_CHEIE` iese din scope. HQ folosește cererea totală a rețelei și se netează independent per `TRDR × MTRL × fereastră`, înainte de clip-ul la zero. **Abatere conștientă**: contractul de intrare al beneficiarului declară coloana `Cod client` drept „Cheie de netting" ([REFERINTA L54](REFERINTA_BENEFICIAR_MINMAX_2026-09-10.md)), iar excluderile sunt deja implementate pe `TRDR.CODE` fiindcă `MECDIS`/`MECDI2`/`INTE79` au câte două înregistrări `TRDR`. Motorul rămâne deci cu excludere pe CODE și netting pe ID; consecință acceptată: un retur pe al doilea `TRDR` al aceluiași cod nu compensează vânzarea de pe primul.
...
5. **P11**: baza TREND rămâne 13S/26S ca fracție sau se adoptă formula 13S/52S din S 7?
6. **P12**: regula explicită ABC/z/SL pentru NOU; N6 nu este această clarificare.
7. **P13**: `MIN_DOC` este global sau per filială și cum se definește ultima vânzare după retururi? P13 se implementează după P4, fiindcă P4 reconstruiește universul `#Items`.
