...
| P2 | Netting pe `TRDR`, nu `TRDR.CODE`; HQ din neturi de filială (D13) | M/U | Secțiunea 7 din [01_classify.sql](../sql/01_classify.sql): filialele și HQ sunt materializate în `#ClientWindowTotals`, apoi agregate în `#BranchWindowTotals` | **DECIS și implementat local (16.09.2026):** cheia rămâne `TRDR`; HQ este scope propriu al cererii. Pentru HQ, neturile brute din toate filialele se însumează per `TRDR × MTRL × fereastră`, apoi se aplică clip-ul la zero. Nu se însumează VZ-urile filialelor deja clip-uite. Seria săptămânală pentru sigma rămâne distinctă și neschimbată | — | T1 + regresie cross-branch `+10/-6`: filiale `10/0`, HQ `4` | După P1; implementat local, necesită deploy și validare SQL live |
