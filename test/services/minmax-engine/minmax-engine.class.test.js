...
describe('minmax-engine service (unit, HTTP mocked)', () => {
  ...
  describe('history()', () => {
    it('clamps the limit to the hard cap of 50', async () => {
      ...
      assert.ok(capturedBody.sqlQuery.includes('CONVERT(VARCHAR(19), STARTEDAT, 126) AS STARTEDAT'))
      assert.ok(capturedBody.sqlQuery.includes('CONVERT(VARCHAR(19), FINISHEDAT, 126) AS FINISHEDAT'))
      assert.ok(capturedBody.sqlQuery.includes('CONVERT(VARCHAR(19), GROUP_STARTEDAT, 126) AS GROUP_STARTEDAT'))
      assert.ok(capturedBody.sqlQuery.includes('CONVERT(VARCHAR(19), COMPUTE_STARTEDAT, 126) AS COMPUTE_STARTEDAT'))
      assert.ok(capturedBody.sqlQuery.includes("WHEN STATUS = 'RUNNING' THEN DATEDIFF(SECOND, STARTEDAT, GETDATE()) ELSE DURATA_SEC END AS CLASSIFY_DURATA_SEC"))
      assert.ok(capturedBody.sqlQuery.includes("WHEN SESSION_STATUS = 'OPEN' THEN DATEDIFF(SECOND, STARTEDAT, GETDATE())"))
